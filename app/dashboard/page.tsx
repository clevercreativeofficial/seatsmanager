...
import { ChevronLeft, ChevronRight, LayoutGrid, List, LogOut, Armchair } from 'lucide-react';
...
const generateTables = (): Table[] => {
    ...
    for (let i = 1; i <= 128; i++) {
        const label = `T${i.toString().padStart(3, '0')}`;
        const seats: Seat[] = Array.from({ length: 8 }, (_, j) => ({
            id: `S${(i - 1) * 8 + j + 1}`.padStart(5, '0'),
            guest: i <= 5 && j <= 3 ? `Guest ${i}-${j + 1}` : null,
        }));

        tables.push({ id: label, label, seats });
    }
    return tables;
};
...
export default function DashboardPage() {
    ...
    const filteredTables = useMemo(() => {
        return tables.filter((table) => {
            const filled = table.seats.filter((s) => s.guest !== null).length;
            ...
            prevTables.map(table => {
                ...
                return {
                    ...
                    seats: table.seats.map(seat =>
                        seat.id === activeSeatId ? { ...seat, guest: guestName } : seat
                    )
                };
            })
        );
    ...
    const removeGuest = (seatId: string) => {
                                ...
                                <ul className="mt-4 space-y-2">
                                    {selectedTable.seats.map((seat) => (
                                        <li key={seat.id} className="flex justify-between items-center">
                                            ...
                                            <span className="text-sm">{seat.guest || 'Empty'}</span>
                                            {seat.guest ? (
                                                <Button
                                                    variant="destructive"
                                                    size="sm"
                        ...
                        <div>
                            ...
                            <p className="text-gray-600">
                                {filteredTables.length} tables found ({filteredTables.reduce((acc, table) =>
                                    acc + table.seats.filter(s => s.guest !== null).length, 0)} guests seated)
                            </p>
                        </div>
